...
def get_loader_without_gt(args):
    ...
    ## test dict part
    nii_files = glob.glob(args.data_root_path + '/**_img.nii.gz')
    # print("nii_files:::::::")
    # print(nii_files)
...
# Customize this npy file format:
class LoadNumpyFile(MapTransform):
    ...
    def __call__(self, data):
        ...
        for key in self.keys:
            npy_file = d[key]  # get the file path
            d[key] = np.load(npy_file)  # load the .npy file as a numpy array
        return d
...
#             # Add metadata similar to LoadImaged
#             if 'filename_or_obj' not in d:  # Ensure filename_or_obj is added only once
#                 d['filename_or_obj'] = npy_file  # Store the file path for later saving
#         return d

def get_loader_without_gt_npy(args, file_path=""):
    val_transforms = Compose(
        [
            LoadNumpyFile(keys=["image"]),
            # LoadImaged(keys=["image"]),
            AddChanneld(keys=["image"]),
            Orientationd(keys=["image"], axcodes="RAS"),
            # ToTemplatelabeld(keys=['label']),
    ...
    ## test dict part
    nii_files = ["/home/local/ASURITE/longchao/Desktop/project/GE_health/CLIP-Driven-Universal-Model/data_temp/img/CT_Abd_FLARE22_Tr_0001-050.npy"]
