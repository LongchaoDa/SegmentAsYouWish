...
class DiscreteGroupImageCanonicalization(torch.nn.Module):
    ...
    def canonicalize(
        ...
        # print(x.shape, group_element_dict["rotation"])

        rotation_angle = -group_element_dict["rotation"].to(device)
        x = K.geometry.rotate(x, rotation_angle, padding_mode='border')#.contiguous()
