...
for epoch in range(start_epoch, num_epochs):
    ...
    for step, batch in enumerate(pbar):
        ...
        # forward proces 接收图像和token，然后计算预测的分割结果 (two losses)
        # seg_loss： eval model的预测的分割结果，vs 真实标签 ==》 calculate 2个重叠程度 （seg_loss small ==》 overlap large ==》 better）
        # ce_loss（Binary Cross-Entropy Loss）： eval 每个像素的预测概率prob vs 真实标签之间的差异
