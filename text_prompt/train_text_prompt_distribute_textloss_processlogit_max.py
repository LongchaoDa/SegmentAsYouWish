...
parser.add_argument(
    ...
    '--tr_npy_path',
    type=str,
    default= "/home/ec2-user/textSegment/MedSAM/data/npy/CT_Abd",
...
parser.add_argument(
    ...
    default="/home/ec2-user/textSegment/MedSAM/work_dir/MedSAM/medsam_vit_b_full.pth",
    # required=True
)
parser.add_argument(
    '-work_dir',
    type=str,
    default="/home/ec2-user/textSegment/Proposal/MedSAM/extensions/text_prompt/train_text_prompt",
...
with open('/home/ec2-user/textSegment/Proposal/MedSAM/extensions/text_prompt/organ_labels.json', 'r') as file:
...
class NpyDataset(Dataset): 
    ...
    def __getitem__(self, index):
        ...
        return {
            ...
            # dlc: Here the text is the label text (extend the image to more informative way)
            "text": [caption],
            # dlc: Here the token is the text token
            "token": text_token,
            "image_name": img_name,
            # dlc: v2 - add label id (0 / 1 / 2 / 3 / ... )
            "label": label_id,
...
if torch.cuda.device_count() > 1:
    ...
    medsam_model = DataParallel(medsam_model)
...
# dlc: distribute process
optim_params = list(
    medsam_model.module.prompt_encoder.text_encoder_head.parameters()
) + list(
    medsam_model.module.mask_decoder.parameters()
) + list(
    medsam_model.module.text_classification_head.parameters()
) if torch.cuda.device_count() > 1 else list(
    medsam_model.prompt_encoder.text_encoder_head.parameters()
) + list(
    medsam_model.mask_decoder.parameters()
) + list(
    medsam_model.text_classification_head.parameters()
)
...
# dlc: handle distribute process
if resume:
    checkpoint = torch.load(resume)
    if torch.cuda.device_count() > 1:
        medsam_model.module.load_state_dict(checkpoint["model"])
    else:
        medsam_model.load_state_dict(checkpoint["model"])
    optimizer.load_state_dict(checkpoint["optimizer"])
    start_epoch = checkpoint["epoch"] + 1
...
for epoch in range(start_epoch, num_epochs):
    ...
    for step, batch in enumerate(pbar):
        ...
        medsam_pred, text_classification_logits = medsam_model(image, tokens)


        # loss = seg_loss(medsam_pred, gt2D) + ce_loss(medsam_pred, gt2D.float())
        
        # dlc v2: 
        seg_loss_value = seg_loss(medsam_pred, gt2D) + ce_loss(medsam_pred, gt2D.float())
        
        # assert text_classification_logits.shape == (batch_size, 13), f"Expected logits shape [(batch_size, 13)], got {text_classification_logits.shape}"
        # assert labels.shape == (batch_size,), f"Expected labels shape [(batch_size,)], got {labels.shape}"

        classification_loss_value = classification_loss(text_classification_logits, labels - 1)

        loss = seg_loss_value + classification_loss_value
        ...
        # Record the loss values to TensorBoard
        writer.add_scalar('Loss/seg_loss', seg_loss_value.item(), epoch * len(train_loader) + step)
        writer.add_scalar('Loss/classification_loss', classification_loss_value.item(), epoch * len(train_loader) + step)
        writer.add_scalar('Loss/total_loss', loss.item(), epoch * len(train_loader) + step)

    epoch_end_time = time()
    epoch_time.append(epoch_end_time - epoch_start_time)
    epoch_loss_reduced = sum(epoch_loss) / len(epoch_loss)
    losses.append(epoch_loss_reduced)
    model_weights = medsam_model.module.state_dict() if torch.cuda.device_count() > 1 else medsam_model.state_dict()
    # checkpoint = {
    #     "model": model_weights,
    #     "epoch": epoch,
    ...
    # dlc: distribute process: 
    checkpoint = {
    "model": medsam_model.module.state_dict() if torch.cuda.device_count() > 1 else medsam_model.state_dict(),
    "epoch": epoch,
    "optimizer": optimizer.state_dict(),
    "loss": epoch_loss_reduced,
    "best_loss": best_loss
    ...
    writer.add_scalar('Time/epoch_time', epoch_end_time - epoch_start_time, epoch)
