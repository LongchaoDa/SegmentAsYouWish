...
avoid_list = ['right kidney', 'Right Adrenal Gland']
left_only = False
...
class MedSAM(nn.Module):
    ...
    def forward(self, image, tokens):
        ...
        # text_classification_probs = torch.softmax(text_classification_logits, dim=1)
        text_classification_probs = F.softmax(text_classification_logits, dim=2)
        
        
        return low_res_masks, text_classification_probs, text_embeddings
...
def aggregate_pkl_data(pkl_data):
    if left_only:
        ...
        return aggregated_data, keep_list
    else:
        aggregated_data = {}
        list_all = list(pkl_data['s'].keys())
        for key in list_all:
...
model_path = "/home/ec2-user/textSegment/Proposal/MedSAM/extensions/text_prompt/eval/test_ckps/medsam_text_prompt_epoch_500_Lable.pth"

name_tag = model_path.split("/")[-1].split("_")[-1].replace(".pth", "")
...
with torch.no_grad():
    for tokens, label in dataloader:
        ...
        _, text_classification_probs, text_embeddings = medsam_model(None, tokens)
        ...
        # predict
        get_probs = text_classification_probs.mean(dim=1) # 合并77 feature 的信息，从[16, 77, 13] 得到： [16, 13]
        
        # get_probs, _ = text_classification_probs.max(dim=1)
        _, final_preds = get_probs.max(dim=1)  # 结果形状为 [16]


        accuracy = accuracy_score(label.cpu().numpy(), final_preds.cpu().numpy())
        # print(text_classification_probs)
        all_accs.append(accuracy)
        
ave_acc = np.mean(np.array(all_accs))
print(f'acc: {ave_acc:.4f}')
...
# plt.savefig("/home/ec2-user/textSegment/Proposal/MedSAM/extensions/text_prompt/eval/save/umap_maxpool30.png", dpi=400)
# # plt.show()
...
# 设置图形标题和标签
ax.set_title(f'{name_tag} Silhouette Score: {sil_score:.2f}, Acc: {ave_acc:.2f}')
...
plt.savefig("/home/ec2-user/textSegment/Proposal/MedSAM/extensions/text_prompt/eval/save/pca_"+name_tag+"_"+tag_extra+"_acc.png", dpi=400)
# plt.show()