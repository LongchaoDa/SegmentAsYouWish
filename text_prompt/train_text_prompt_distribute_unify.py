...
if augmented_textlabel:
    with open('text_prompt/organ_labels.json', 'r') as file:
        label_dict = json.load(file)
else: 
    with open('text_prompt/organ_labels_pure.json', 'r') as file:
        label_dict = json.load(file)
...
if torch.cuda.device_count() > 1:
    optim_params =  list(medsam_model.module.prompt_encoder.text_encoder_head.parameters()) + \
                    list(medsam_model.module.mask_decoder.parameters()) + \
                    list(medsam_model.module.text_classification_head.parameters()
                    ) 
else:
    optim_params =  list(medsam_model.prompt_encoder.text_encoder_head.parameters()) + \
                    list(medsam_model.mask_decoder.parameters()) + \
                    list(medsam_model.text_classification_head.parameters()
                    )
