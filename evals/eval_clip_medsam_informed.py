    ...
    with torch.no_grad(): 
        ...
        for step, batch in enumerate(pbar):
            ...
            for i in range(medsam_segs.shape[0]):
                embedding = medsam_model.image_encoder(torch.from_numpy(medsam_segs[i:i+1]).to(device).unsqueeze(1) * img)
                cropped_img_embeddings.append(embedding.mean((-1,-2)))
            cropped_img_embeddings = torch.cat(cropped_img_embeddings, dim = 0)

            # Tokenize the batch of text descriptions
            inputs = processor(text=text[0], return_tensors="pt", padding=True)
            # Move inputs to the GPU (if available)
            inputs = {key: value.to(device) for key, value in inputs.items()}
            # Generate text embeddings
            with torch.no_grad():  # Disable gradient calculation since it's not needed for inference
                text_embeddings = model.get_text_features(**inputs).reshape(1,256,2).mean(-1)
