...
modality = "CT"
anatomy = "BTCV"  # anantomy + dataset name
subset = "Abdomen"
img_name_suffix = ".nii.gz"
gt_name_suffix = ".nii.gz"
prefix = modality + "_" + anatomy + "_"

# img_name = "img0001.nii.gz"
# gdt_name = "label0001.nii.gz"

nii_path = f"/home/ec2-user/SageMaker/SegmentAsYouWish/text_prompt/data/touse/BTCV/unzip/{subset}/RawData/Training/img"  # path to the nii images
gt_path = f"/home/ec2-user/SageMaker/SegmentAsYouWish/text_prompt/data/touse/BTCV/unzip/{subset}/RawData/Training/label"  # path to the ground truth
npy_path = "/home/ec2-user/SageMaker/SegmentAsYouWish/text_prompt/data/touse/BTCV/npy/" + prefix[:-1] + f'/{subset}/'
...
names = [
    name
    for name in names
    if os.path.exists(join(nii_path, name.split(gt_name_suffix)[0].replace('label', 'img') + img_name_suffix))
]
...
for name in tqdm(names):  # use the remaining 10 cases for validation
    image_name = name.split(gt_name_suffix)[0].replace('label', 'img') + img_name_suffix
    gt_name = name
    print(f'Processing for img = {image_name}, gdt = {gt_name}')
