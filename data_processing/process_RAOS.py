...
modality = "CT"
anatomy = "RAOS"  # anantomy + dataset name
Category = "MissingOrgans(Set3)" # CancerImages(Set1) AfterSurgery(Set2) MissingOrgans(Set3)
subset = "Ts" # Tr Val Ts

img_name_suffix = "_0000.nii.gz" # .nii.gz/_0000.nii.gz for Ts
gt_name_suffix = ".nii.gz"
prefix = modality + "_" + anatomy + "_"

nii_path = f"/home/ec2-user/SageMaker/SegmentAsYouWish/text_prompt/data/touse/RAOS/unzip/RAOS-Real/{Category}/images{subset}"  # path to the nii images
gt_path = f"/home/ec2-user/SageMaker/SegmentAsYouWish/text_prompt/data/touse/RAOS/unzip/RAOS-Real/{Category}/labels{subset}"  # path to the ground truth
npy_path = "/home/ec2-user/SageMaker/SegmentAsYouWish/text_prompt/data/touse/RAOS/npy/RAOS-Real/" + prefix[:-1] + f'/{Category}/{subset}/'
...
# Overlap check
with open('/home/ec2-user/SageMaker/SegmentAsYouWish/text_prompt/data/touse/RAOS/unzip/RAOS-Real/overlap_with_word.json') as file:
    word2raos_overlap_dict = json.loads(json.load(file))                                        

overlaps = list(word2raos_overlap_dict.values())
print(f'collected raos subj (gt example) = {names[0]}')    #1.2.840.113619.2.416.142665548333154486557858119673677954013.nii.gz
print(f'overlaped raos subj (gt example) = {overlaps[0]}') #'1.3.12.2.1107.5.1.4.66045.30000020022409243120600048800.nii.gz',
names = [
    name
    for name in names
    if  name not in overlaps
]
print(f"after overlap check \# files {len(names)=}")
# exit(1)
